"""
RTMOPose-M (medium) – HGNetV2-B2.
"""

from .include.rtmopose_hgnetv2 import model, criterion, training_params, postprocessor
...
# ── Output / training ─────────────────────────────────────────────────────────
training_params["output_dir"] = "output/rtmopose_hgnetv2_m_custom"
...
optimizer = L(optim.AdamW)(
    params=L(get_optim_params)(
        cfg=[{"params": "^(?=.*backbone).*$", "lr": 0.00005}],
    ),
    lr=0.0001,
    betas=[0.9, 0.999],
    weight_decay=0.0001,
